...
function silentHomeCardDetails(item, isDone, itemMeta) {
  ...
  const map = {
    ...
    test: {
      unit: "확인 문제 2문항",
      review: test.accuracy === null ? "오늘 풀이 기록 대기" : `오늘 정답률 ${test.accuracy}%`,
      reason: test.solved ? `${test.solved}문항 풀이 완료` : "짧게 실력 점검",
    },
    quiz: {
      unit: "실무형 문제 3문항",
      review: quiz.reviewNeeded ? `오답/북마크 ${quiz.reviewNeeded}개 복습 필요` : "복습 대기 없음",
      reason: quiz.accuracy === null ? `최근 약한 유형: ${quiz.weakType}` : `최근 정답률 ${quiz.accuracy}%`,
    },
  };
